...
func TestStoreLoadWithContext(t *testing.T) {
	...
	gatewayConfig := ConfigMapFromTestFile(t, IstioConfigName)
	store.OnConfigChanged(gatewayConfig)
	config := FromContext(store.ToContext(context.Background()))

	t.Run("load ingressgateway", func(t *testing.T) {
		expected, _ := NewIstioFromConfigMap(gatewayConfig)
...
func TestStoreImmutableConfig(t *testing.T) {
	...
	if newConfig.Istio.IngressGateway == "mutated" {
		t.Error("IngressGateway config is not immutable")
	}
}
