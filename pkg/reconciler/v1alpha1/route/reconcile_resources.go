...
func (c *Reconciler) reconcilePlaceholderService(ctx context.Context, route *v1alpha1.Route) error {
	...
	if apierrs.IsNotFound(err) {
		...
		if !equality.Semantic.DeepEqual(service.Spec, desiredService.Spec) {
			...
			origin.Spec = desiredService.Spec

			_, err = c.KubeClientSet.CoreV1().Services(ns).Update(origin)
			if err != nil {
				return err
			}
		}
...
func (c *Reconciler) updateStatus(ctx context.Context, desireRoute *v1alpha1.Route) (*v1alpha1.Route, error) {
	route, err := c.routeLister.Routes(desireRoute.Namespace).Get(desireRoute.Name)
	if err != nil {
		return nil, err
	}
	// If there's nothing to update, just return.
	if reflect.DeepEqual(route.Status, desireRoute.Status) {
		return route, nil
	}
	// Don't modify the informers copy
	existing := route.DeepCopy()
	existing.Status = desireRoute.Status
	// TODO: for CRD there's no updatestatus, so use normal update.
	updated, err := c.ServingClientSet.ServingV1alpha1().Routes(desireRoute.Namespace).Update(existing)
	if err != nil {
		return nil, err
	}

	c.Recorder.Eventf(route, corev1.EventTypeNormal, "Updated", "Updated status for route %q", desireRoute.Name)
	return updated, nil
}
